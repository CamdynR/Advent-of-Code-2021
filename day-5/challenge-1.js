...
/**
 * Filters out all of the diagonal lines from the given input
 * @param {array<array>} input 3D array of coordinate pairs
 * @returns {array<array>} Same input, but all of the diagonal lines removed
 */
function filterDiagonals(input) {
  return input.filter(coordPair => {
    const xEqual = coordPair[0][0] == coordPair[1][0];
    const yEqual = coordPair[0][1] == coordPair[1][1];
